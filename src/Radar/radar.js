...
export default {
  draw(id, json, options) {
    ...
    function updateConfig() {
      ...
      config.maxValue = Math.max(config.maxValue, max(data, (d) => max(d.axes, (o) => o.value)));
    ...
    function buildVisComponents() {
      ...
      vis.allAxis = data[0].axes.map((i, j) => i.axis);
      ...
      vis.verticesTooltip = select('body')
    ...
    function buildLevels() {
      for (let level = 0; level < config.levels; level++) {
        ...
        vis.levels
          ...
          .attr('x1', (d, i) => levelFactor * (1 - Math.sin(i * config.radians / vis.totalAxes)))
          .attr('y1', (d, i) => levelFactor * (1 - Math.cos(i * config.radians / vis.totalAxes)))
          .attr('x2', (d, i) => levelFactor * (1 - Math.sin((i + 1) * config.radians / vis.totalAxes)))
          .attr('y2', (d, i) => levelFactor * (1 - Math.cos((i + 1) * config.radians / vis.totalAxes)))
    ...
    function buildLevelsLabels() {
      for (let level = 0; level < config.levels; level++) {
        ...
        vis.levels
          ...
          .attr('x', (d) => levelFactor * (1 - Math.sin(0)))
          .attr('y', (d) => levelFactor * (1 - Math.cos(0)))
    ...
    function buildAxesLabels() {
      vis.axes
        ...
        .attr('x', (d, i) => config.w / 2 * (1 - 1.3 * Math.sin(i * config.radians / vis.totalAxes)))
        .attr('y', (d, i) => config.h / 2 * (1 - 1.1 * Math.cos(i * config.radians / vis.totalAxes)))
        .attr('font-family', 'sans-serif')
    ...
    function buildPolygons() {
      vis.vertices
        ...
        .attr('points', (group) => { // build verticesString for each group
          let verticesString = '';
          group.axes.forEach((d) => { verticesString += `${d.coordinates.x},${d.coordinates.y} `; });
          return verticesString;
        })
        .attr('stroke-width', '2px')
        .attr('stroke', (d, i) => config.colors(i))
        ...
        .on(over, function (d) {
          ...
          select(this) // focus on active polygon
            .transition(250)
    ...
    function buildLegend() {
      ...
      vis.legend.selectAll('.legend-tiles')
        ...
        .attr('y', (d, i) => i * 2 * config.legendBoxSize)
        .attr('width', config.legendBoxSize)
        .attr('height', config.legendBoxSize)
        .attr('fill', (d, g) => config.colors(g));
